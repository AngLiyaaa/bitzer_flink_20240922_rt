...
//        sensorKS.window(SlidingProcessingTimeWindows.of(Time.seconds(10), Time.seconds(2))) //滚动窗口，窗口长度10s,滑动步长2s
//        sensorKS.window(ProcessingTimeSessionWindows.withGap(Time.seconds(5))) //会话窗口，超时间隔5s

//        基于计数的
//        sensorKS.countWindow(5) // 滚动窗口，窗口长度=5个元素
//        sensorKS.countWindow(5,2) // 窗口长度=5个元素，滑动步长=2个元素
//        sensorKS.window(GlobalWindows.create()) //全局窗口，计数窗口的底层用的就是这个，需要自定义触发器，很少用
...
//        增量聚合： 来一条数据，计算一条数据，串口触发的时候输出计算结果
//        sensorWS
//                .reduce()
//                .aggregate()
